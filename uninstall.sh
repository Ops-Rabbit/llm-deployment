...
Removes the GLM service and API proxy. Model files and cache data are preserved.
...
rm -f /etc/opsrabbit-llm/sglang-auth.json /etc/opsrabbit-llm/vllm.env /run/opsrabbit-llm/vllm.sock
...
printf 'GLM service removed. Model data was preserved at: %s\n' "$data_dir"
